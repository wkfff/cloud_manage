...
<template>
  <div>
    <CloudForm :fields="fieldsProps" :config="configProps" ref="db2Authorization"/>
  </div>
</template>
...
export default {
    ...
    fieldsProps() {
      ...
      return [
        ...
        },
        {
          ...
          events: {
            change: val => {
              const valid = _this.$refs["db2Authorization"].validateField(
                "ip",
                valid => {
                  if (!valid) {
                    console.log(val);
                    _this.getInstances(val, "ip");
                    _this.$refs["db2Authorization"].setFieldValue({
                      object_owner: "",
                      instance_name: ""
                    });
                  }
  ...
  created() {
    if (
      Object.prototype.toString.call(this.permissions) === "[object Array]" &&
