...
<template>
  <div>
    <CloudForm :fields="fieldsProps" :config="configProps" ref="sqlserverWhitelist"/>
  </div>
</template>
...
import { SQLSERVER_AUTHORITY_TYPES } from "@/utils/constant";

export default {
  name: "sqlserverWhitelist",
    ...
    fieldsProps() {
      ...
      return [
        ...
        },
        {
          ...
          events: {
            change: val => {
              const valid = _this.$refs["sqlserverWhitelist"].validateField(
                "ip",
                valid => {
                  if (!valid) {
                    console.log(val);
                    _this.getInstances(val, "ip");
                    _this.$refs["sqlserverWhitelist"].setFieldValue({
                      instance_name: "",
                      object_owner: ""
                    });
                  }
  ...
  methods: {
    ...
    async getAuthorityTypes() {
      const res = await this.$ajax.get(GET_WORKSHEET_DICTIONARY, {
        type: "SQLSERVER_WHITE_LIST_RIGHT"
      });
      this.authoritys = res.data.map(v => {
        return { ...v, label: SQLSERVER_AUTHORITY_TYPES[v.name], value: v.name };
      });
    },
